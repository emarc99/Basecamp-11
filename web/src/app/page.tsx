...
import { useBlockNumber, useAccount, useBalance, useReadContract, useNetwork, Abi, useContract, useSendTransaction, useTransactionReceipt} from '@starknet-react/core';
import { ABI } from "../abis/abi";
import { Contract, RpcProvider, transaction } from 'starknet';
...
const Page: FC = () => {
  ...
  const {data: blockNumberData, isError: blockNumberError, isLoading: blockNumberLoading} = useBlockNumber({
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 flex flex-col">
      <h1 className="text-3xl font-bold text-center mb-6">Basecamp 11 Starknet Frontend Workshop</h1>
            ...
            <table className="w-full border-collapse">
              ...
              {lastFiveEvents.map((event, index) => (
                    <tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                      ...
                      <td className="border-b border-gray-200 p-2 text-right">{event.data.length > 0 ? formatAmount(event.data[0]): 'Original value'}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
